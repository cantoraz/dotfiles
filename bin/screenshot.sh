...
capture() {
  ...
  if [ $SHOT_COPY = 0 ]; then
    ...
    chmod $(printf '%03o' $((8#666-8#$(umask)))) "$SHOT_PATH"
  else
...
notify() {
  local notify_options="-a '$NOTIFY_APP_NAME' -h $NOTIFY_CATEGORY"
  [ ! -v ERR ] && notify_options+=" -I $SHOT_TMP"
  dunstify $notify_options "$NOTIFY_SUMMARY" "$NOTIFY_BODY"
}
